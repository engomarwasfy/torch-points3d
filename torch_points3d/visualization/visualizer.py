...
class Visualizer(object):
    ...
    def __init__(self, viz_conf, num_batches, batch_size, save_dir, tracker):
        ...
        if "tensorboard" in self._format and tracker._use_tensorboard:
    ...
    def get_indices(self, stage):
        ...
        if stage_num_batches > 0:
            if self._num_samples_per_epoch < 0:  # All elements should be saved.
                ...
                self._deterministic
                and stage not in self._indices
                or not self._deterministic
            ):
    ...
    def _extract_from_PYG(self, item, pos_idx):
        ...
        return {
            ...
            and (
                k in self._saved_keys.keys()
                or k in self._tensorboard_mesh.values()
            )
            and item[k].shape[0] == num_samples
        }
    ...
    def save_visuals(self, visuals):
        ...
        """
        if self._stage not in self._indices:
            return
